...
{
    public class cls_SQLOperateCusPJ
    {
        ...
        {
            return function.SQLReadData("SELECT * FROM T_Users WHERE FUserID='" + FUserID + "'", DBConStr);
        }
        ...
        {
            return function.SQLReadData("SELECT FState,FPower FROM T_Users WHERE FUserID='" + UserID + "'", ConStr);
        }
        ...
        {
            return function.SQLReadData("SELECT 'PL'+RIGHT('000000'+CONVERT(NVARCHAR(10),FID),6) AS Code,* FROM T_WorkPlan WHERE FID='"+ FID + "'", DBConStr);
        }
        ...
        public DataTable ReadWorkPlanProcList(string FID, string UserProcIDList, string DBConStr)
        {
            ...
            string SQL = string.Format(@"SELECT t1.FItemID,t1.FSeriaNumber,t2.FName,t1.FIDProcedure,ISNULL(t1.FIsOut,'N') FIsOut,ISNUll(t1.FIDOutUnit,'') FIDOutUnit,t1.FRemark
                FROM T_WorkPlan_Pro t1 
                LEFT JOIN T_Procedure t2 ON t1.FIDProcedure=t2.FID
                WHERE t1.FIsOut='N' AND t1.FIDWorkplan={0} {1}
                GROUP BY t1.FItemID,t1.FSeriaNumber,t2.FName,t1.FIDProcedure,t1.FIsOut,ISNUll(t1.FIDOutUnit,''),t1.FRemark ORDER BY t1.FSeriaNumber", FID, ProcStr);
        ...
        // 读取生产计划某工序的累计已报工数量
        public DataTable ReadWorkPlanProcTotalQty(string WrokPlanProcID, string DBConStr)
        {
            return function.SQLReadData("SELECT ISNULL(SUM(FCounts),0) AS FCounts,ISNULL(SUM(FBad),0) AS FBad,ISNULL(SUM(FLoss),0) AS FLoss FROM T_Production_Data WHERE FIDWorkPlanPro=" + WrokPlanProcID, DBConStr);
        }

        // 根据生产计划ID读取销售订单信息
        public DataTable ReadWorkPlanSaleOrderInfo(string FWorkPlanID, string DBConStr)
        {
            string SQL = string.Format(@"SELECT tSOD.FSituation FROM T_WorkPlan tWP
                LEFT JOIN T_SaleOrderDetail tSOD ON tWP.FItemID=tSOD.FDetailID
                WHERE FID={0}", FWorkPlanID);
            return function.SQLReadData(SQL, DBConStr);
        }
