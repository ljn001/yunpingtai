...
namespace CloudPlatform.Code
{
    public class cls_SQLFunction
    {
        ...
        System.Configuration.ConnectionStringSettings connString_Self = System.Configuration.ConfigurationManager.ConnectionStrings["SelfConnectionString"];
        System.Configuration.ConnectionStringSettings connString_Selfxinxi = System.Configuration.ConfigurationManager.ConnectionStrings["SelfConnectionStringxinxi"];
        // 读取数据库
        public DataTable SQLReadData(string ComTxt, string conStr = null)
        {
            try
            {
                ...
                {
                  if(conStr=="人员")//取人事数据库中的人员信息存放wxopenid，找对应部门。然后去找对应部门对应的数据库；
                        connectionString = connString_Selfxinxi.ConnectionString;
                  else
