<%@ Page Language="C#" AutoEventWireup="true" CodeBehind="HBLog.aspx.cs" Inherits="CloudPlatform.m.HBLog" %>
<!DOCTYPE html>
...
<body class="f-body f-widget-content f-mobi f-webkit f-safari f-ios f-iphone f-theme-pure_blue">
                                ...
                                <div class="card-header">
                                    <div class="row align-items-center">
                                        <div class="col align-self-center  pr-0">
                                            ...
                                            <p class="text-secondary"><%# DateTime.Parse(Eval("FEnd").ToString()).ToString("yyyy-MM-dd HH:mm") %></p>
                                        </div>
                                        <div class="col-auto align-self-center pl-0">
                                            <a class="btn btn-sm btn-danger rounded" style="color:#fff" onclick="OnDeleteClick('<%# Eval("FPLNo") %>','<%# Eval("FItemID") %>');">删除汇报</a>
                                        </div>
                                    </div>
                                </div>
                                <div class="card-body bg-warning-light" style="padding:5px;">
                                    <h1 class="text-center mb-0"><%# Eval("FCounts") %></h1>
                                </div>
                                <div class="card-footer  bg-warning text-white">
                                    <div class="row align-items-center">
                                        <div class="col">
                                            <p class="mb-0"><%# Eval("FPLNo") %></p>
                                            <p class="mb-1">计划单</p>
                                        </div>
                                        <div class="col-auto align-self-center text-right">
                                            <p class="mb-0"><%# Eval("FBillNo") %></p>
                                            <p class="mb-1">客户订单</p>
                                        </div>
                                    </div>
                                </div>
    ...
    <script>
        function OnDeleteClick(planNo,id) {
            pxmu.diaglog({
                title: { text: '操作确认' },
                ...
                congif: {
                    ...
                    if (res.btn == 'right') {
                        F.customEvent('delete|' + planNo + '|' + id);
                    }
                    else {
